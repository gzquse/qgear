...
def con_to_ang(data,nq_addr,nq_data):
    '''
    Changing the pixel values to bit string and collecting and grouping the bit values from
    under the various bit positions in the bit string.  So group all the 1st bit postion values
    from the bit strings generated from the pixel values under one column vector. For bit
    position i in all the pixels, we record those under column i in the matrix generated
    '''
...
def rescale_angles_to_idata(angles, max_val=256):  # keep name for backward compatibility
