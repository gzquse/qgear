        ...
        for kk in range(k):
            for i in range(pdfs.shape[0]):
                if pdfs[i, kk] > 0:
                    loc = get_bits(
                        i,
                        # list(range(self.nq_data, self.nq_data + self.nq_addr)) # TODO : reverse the bits in loc for this option
                        list(range(0, self.nq_addr))
                    )
                    if data[loc, kk] == 0:
                        data[loc, kk] = get_bits(
                            i,
                            # list(range(0, self.nq_data)) # TODO : reverse the bits for this option
                            list(range(self.nq_addr, self.nq_data + self.nq_addr))
                        )
        return data
