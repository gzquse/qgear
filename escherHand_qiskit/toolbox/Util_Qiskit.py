...
def qiskit_to_cudaq(qcs, shots):
    ...
    print('cuda quantum job started, nCirc=%d  nq=%d  shots/circ=%d at %s ...'%(len(qcs),qc1.num_qubits,shots,'cuda_quantum'))
    for quantum_data1 in qcs:
        quantum_data=transpile(quantum_data1,backend)
        qregs=quantum_data.qregs
        print('Qregs;',qregs)
        ...
        print('CCC, nq=',qubit_count)
        # Translate Qiskit operations to CUDAQ
        for instruction in quantum_data:
            print('iii',instruction)
            gate = instruction.operation.name
            params = instruction.operation.params
            qubits=  instruction.qubits[0]
            print('qqq',qubits)
            print('qqq2',dir(qubits))
            print('qq3',qubits._index)
            ...
            if gate == 'h':
                ...
                kernel.ry(params[0], qubits[qubit_indices[0]])
            elif gate == 'measure':
