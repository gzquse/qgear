...
# %% auto 0
__all__ = ['prepare_runner_args', 'run_qiskit_aer', 'run_cudaq', 'run_cudaqft', 'input_shard', 'run_gate_job', 'rank_print',
           'canned_qcrank_inp', 'make_qcrank', 'harvest_cudaq_backRun_submitMeta', 'run_qcrank']
...
# %% ../nbs/runner.ipynb 4
def run_cudaq(gateD, shots, target, num_qpus, nc=1, verb=1, backend="qpp-cpu"):
    if backend == "qpp-cpu":
        cudaq.set_target(backend)
    else:
        cudaq.set_target(backend, option="mgpu")
    "Run CUDA-Q circuits from gate list"
    resL = [0] * nc
    for i in range(nc):
