...
from toolbox.Util_CudaQ import qiskit_to_cudaq, counts_cudaq_to_qiskit, cudaq_run
...
if __name__ == "__main__": 
    ...
    for i in range(nCirc):        
        qKerL[i]=qiskit_to_cudaq(qcL[i], nq)
    ...
    try:
        ...
        resL = cudaq_run(qKerL, resL, shots, gpu_count)
        elaT=time()-T0
    except Exception as e:
        log.error("Cuda sampling error: %s", e, exc_info=True);raise
    log.info('M:  ended elaT=%.1f sec'%(elaT))
    try:
        resL = counts_cudaq_to_qiskit(resL)
    except Exception as e:
        log.error("counts string convertion error: %s", e);raise
